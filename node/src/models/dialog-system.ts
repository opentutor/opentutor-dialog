...
export async function processUserResponse(
  ...
  if(prompt.length>0)
  {
    ...
    if(expectationResults[sdp.dialogState.expectationsCompleted.indexOf(false)].evaluation === Evaluation.Good)
    {
      //prompt completed successfully
      sdp.dialogState.expectationsCompleted[sdp.dialogState.expectationsCompleted.indexOf(false)] = true;
  ...
  if(hintText.length > 0) {
    ...
    if(expectationResults[expectationId].evaluation === Evaluation.Good){
      ...
      const prompt: Prompt = atd.prompts.find( p => p.expectationId == expectationId);
