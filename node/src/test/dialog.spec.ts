...
describe('dialog', () => {
  ...
  describe('POST', () => {
    ...
    it('responds with a 502 error if 500 error calling classifier', async () => {
      if (mockAxios) {
        ...
        mockAxios.onPost('/classifier').reply( config  => {
          ...
          console.log('here');
          console.log(reqBody);
          // expect(reqBody).to.have.property('question', lessonId);
          // expect(reqBody).to.have.property('inputSentence', 'peer pressure');
          return [500, {}];
        });
    ...
    it('sends the session information when session is started, along with initial dialog', async () => {
      ...
      console.log(JSON.stringify(response.body.sessionInfo));
      expect(response.status).to.equal(200);
